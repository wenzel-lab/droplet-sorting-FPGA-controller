...
def read(mem, size):
    ...
    global log_buffer, voltage_buffer, last_droplet_id, time_voltage_ms, voltage_points, enabled_channels, active_channels
    
    flag_change = 0

    try:
        # Read key registers
        current_droplet_id = read_register(mem, addr_droplet_id, size, signed_droplet)
        current_time_voltage_ms = read_register(mem, addr_signal_duration, size, signed_signal_duration)
        current_enabled_channels_int = read_register(mem, addr_enabled_channels, size, signed_enabled_channels)
        ...
        # Read voltages for all channels and store values of active ones
        voltage_per_channel = [analog_voltage(read_register(mem, ch_voltage_addr, size, signed_voltage),signed_voltage) for ch_voltage_addr in addrs_cur_adc_data]
